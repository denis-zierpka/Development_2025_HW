...
*Комментарий: предварительно нужно проверить запущен ли minikube*

Для автоматического развёртывания всей системы выполнить команды:
```
chmod +x deploy.sh
./deploy.sh
```
...
1. Пробросить порты командой: `kubectl port-forward service/app-service 8080:80`

    И далее выполнять команды:

    ```
    curl http://localhost:8080/
    curl http://localhost:8080/status
    curl -s -X POST http://localhost:8080/log --json '{"message": "check"}'
    curl http://localhost:8080/logs
    ```


2. Использовать дополнительный контейнер: `kubectl run test-curl --rm -it --image=curlimages/curl -- sh`

    Из которого можно будет проверить, например, балансировку (выполнив сразу несколько запросов):

    ```
    for i in {1..10}; do curl -s -X POST http://app-service/log --json '{"message": "test"}'; done
    ```

    Результаты распределения запросов можно посмотреть в логах pod-ов (`kubectl get pods`), через команду `kubectl logs <pod-name>`
